...
const PdfViewer = dynamic(() => import("../components/PdfViewer"), {
  ssr: false,
  loading: () => (
    <div className="h-full w-full flex items-center justify-center bg-neutral-100 text-neutral-400 text-xs">
      Loading viewer...
    </div>
  ),
});
...
export default function OwoPage() {
  ...
  const [currentDetail, setCurrentDetail] = useState<ShipmentDetail | null>(
    null,
  );
  ...
  useEffect(() => {
    const saved = localStorage.getItem("sidebarPosition") as "left" | "right";
    if (saved) setSidebarPosition(saved);
  }, []);

  const handleChangeSidebarPosition = (pos: "left" | "right") => {
      ...
      try {
        ...
        if (res.ok) {
          ...
          if (data.success) {
            ...
            const has10B = fetchedClusters.some(
              (c) =>
                c.sub_cluster ===
                "(10B) Tanggal pada BAPP Tidak Sesuai dengan Web",
            );
            if (!has10B) {
    ...
    const loadCurrentAndPrefetchNext = async () => {
      ...
      if (currentItem.starlink_id) {
        ...
        if (sn.length !== 15) {
          ...
          setSelectedRejections((prev) => {
            ...
            setCustomReason(
              Array.from(new Set(Object.values(next))).join("; "),
            );
            return next;
          });
        }
      ...
      if (prefetchData?.id === currentItem.shipment_id) {
        if (isMounted) {
          ...
          if (
            prefetchData.data.is_duplicate_sn &&
            currentItem.starlink_id?.length === 15
          ) {
          ...
          if (!prefetchData.data.shipment.bapp_number) {
            ...
            const targetOption = clusters.find(
              (c) =>
                c.main_cluster === "Skylink Web" &&
                c.nama_opsi.includes("Nomor BAPP Tidak Ada"),
            );
            const reason = targetOption
              ? targetOption.sub_cluster
              : "(10A) Nomor BAPP Tidak Ada";
        ...
        try {
          const res = await fetch(
            `/api/fetch-data?id=${currentItem.shipment_id}&cutoff_id=${currentItem.id}`,
            {
              headers: {
          ...
          if (!res.ok) {
            throw new Error(
              `Failed to fetch details for shipment ${currentItem.shipment_id}`,
            );
          }
          const data = await res.json();
          if (isMounted) {
      ...
      if (nextItem) {
        try {
          ...
          const res = await fetch(
            `/api/fetch-data?id=${nextItem.shipment_id}&cutoff_id=${nextCutoffItem.id}`,
            {
              headers: {
                Authorization: `Bearer ${localStorage.getItem("access_token") || ""}`,
              },
  ...
  useEffect(() => {
    ...
    if (tanggalBapp && receivedDate && tanggalBapp !== receivedDate) {
      ...
      if (
        currentRejection &&
        currentRejection.includes("Nomor BAPP Tidak Ada")
      ) {
        return;
      }
  ...
  useEffect(() => {
    ...
    if (serialRejection) {
      ...
      if (
        serialRejection.includes("tidak jelas") ||
        serialRejection.includes("tidak ada")
      ) {
  ...
  const handleSubmit = async () => {
    ...
    const finalSerialNumber = isSerialNumberMismatch
      ? manualSerialNumber
      : currentItem.starlink_id;
    ...
    try {
      ...
      setProcessingStatus("error");
      const errorMsg =
        logError.name === "AbortError"
          ? "Request timeout (exceeded 3 seconds)"
    ...
    try {
      ...
      if (res.ok) {
        ...
        alert(
          `Failed to ${finalStatus.toLowerCase()} on Skylink: ${data.error || "Unknown error"}`,
        );
      }
    } catch (error) {
      ...
      alert(
        `An error occurred while trying to ${finalStatus.toLowerCase()} on Skylink.`,
      );
    } finally {
      setIsSubmitting(false);
    }
  ...
  useEffect(() => {
    if (isModalOpenRef.current && currentDetail) {
      ...
      if (firstEvidence) {
        ...
        const isNextPdf = firstEvidence.file_path
          .toLowerCase()
          .endsWith(".pdf");
  ...
  // Navigation Helper
  const switchEvidence = useCallback(
    (direction: "next" | "prev") => {
      ...
      const currentIdx = evidences.findIndex((e) => {
        ...
        return (
          proxyUrl === currentUrl ||
          currentUrl?.includes(encodeURIComponent(e.file_path))
        );
      });
    ...
    return (
      <div className="rounded-2xl border border-neutral-200 bg-white p-12 text-center shadow-sm max-w-lg mx-auto mt-10">
        ...
        <p className="mt-2 text-neutral-500">
          You have no pending items in your workspace. Great job!
        </p>
      </div>
    );
  }
  ...
  if (currentIndex >= queue.length) {
    return (
      <div className="rounded-2xl border border-neutral-200 bg-white p-12 text-center shadow-sm max-w-lg mx-auto mt-10">
        ...
        <p className="mt-2 text-neutral-500">
          You have processed all {queue.length} items in your session.
        </p>
        <button
          onClick={() => {
            setCurrentIndex(0);
  ...
  return (
    <div className="flex h-screen w-full bg-zinc-50 dark:bg-black overflow-hidden relative">
      {/* Sidebar */}
      <div
        className={`flex-shrink-0 h-full ${sidebarPosition === "left" ? "order-1 border-r border-zinc-700" : "order-2 border-l border-zinc-700"}`}
      >
        <Sidebar
          ...
          setPosition={handleChangeSidebarPosition}
          ...
          currentCategory={(() => {
            const currentUrl = selectedImage || selectedPdf;
            if (!currentDetail?.evidences?.data?.length || !currentUrl)
              return undefined;
            ...
            const currentEvidence = currentDetail.evidences.data.find((e) => {
              ...
              return (
                proxyUrl === currentUrl ||
                currentUrl.includes(encodeURIComponent(e.file_path)) ||
                currentUrl.includes(e.file_path)
              );
            });
            ...
            <div className="max-w-5xl mx-auto flex flex-col gap-6">
              ...
              <div className="bg-white dark:bg-zinc-800 rounded-xl shadow-sm border border-zinc-200 dark:border-zinc-700 p-5 relative">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-y-4 gap-x-8">
                  <InfoItem
                    label="Nama Sekolah"
                    value={currentItem.school_name}
                  />
                  <InfoItem label="NPSN" value={currentItem.npsn} />
                  <InfoItem
                    label="Resi Number"
                    value={currentItem.resi_number}
                  />
                  <InfoItem
                    label="Starlink ID"
                    value={currentItem.starlink_id}
                  />
                  <InfoItem
                    label="Received Date"
                    value={new Date(
                      currentItem.received_date,
                  ...
                  <InfoItem
                    label="Status"
                    value={currentDetail?.shipment?.status || "Unknown"}
                  />
                </div>
              </div>
              ...
              <div className="bg-white dark:bg-zinc-800 rounded-xl shadow-sm border border-zinc-200 dark:border-zinc-700 p-5">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-y-4 gap-x-8">
                  <InfoItem
                    label="Shipment ID"
                    value={String(currentItem.shipment_id)}
                  />
                  <InfoItem
                    label="Starlink ID"
                    value={currentItem.starlink_id}
                ...
                <div className="bg-white dark:bg-zinc-800 rounded-xl shadow-sm border border-zinc-200 dark:border-zinc-700 p-5">
                  ...
                  <div className="space-y-3">
                    {history.map((log: any, idx: number) => {
                      const isVerified =
                        log.status?.toUpperCase() === "VERIFIED";
                      const rejections =
                        log.rejections &&
                        typeof log.rejections === "object" &&
                        Object.keys(log.rejections).length > 0
                          ? Object.entries(log.rejections).map(
                              ([k, v]) => `${k}: ${v}`,
                            )
                          : null;
                ...
                {sortedEvidences.length > 0 ? (
                  <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-4">
                    {sortedEvidences.map((ev, idx) => {
                      ...
                      const fileName =
                        ev.file_path.split("/").pop() || "Unknown File";
                            ...
                            <span
                              className={`inline-block px-1.5 py-0.5 rounded text-[10px] font-medium mb-1 ${
                                ev.category === "BAPP"
                                  ...
                                  : ev.category === "PLANG_SEKOLAH"
                                    ? "bg-indigo-100 text-indigo-800 dark:bg-indigo-900/30 dark:text-indigo-400"
                                    : ev.category === "PENERIMA"
                                      ? "bg-green-100 text-green-800 dark:bg-green-900/30 dark:text-green-400"
                                      : ev.category === "SERIAL_NUMBER"
          ...
          >
            ...
            <div className="flex-none p-4 bg-neutral-900 border-b border-neutral-800 flex items-center justify-between shadow-md z-10">
              <div className="text-white text-sm font-medium truncate max-w-md flex items-center gap-2">
                <span className="bg-neutral-800 px-2 py-0.5 rounded text-xs text-neutral-400 border border-neutral-700">
                  Preview
                </span>
                {selectedPdf
                ...
                {selectedImage && (
                  <>
                    <div className="flex items-center bg-neutral-800 rounded-lg p-1 border border-neutral-700">
                      ...
                      <span className="text-xs font-mono text-neutral-300 min-w-[3.5ch] text-center select-none">
                        {zoom.toFixed(1)}x
                      </span>
                      <button
                    ...
                    <button
                      onClick={handleReset}
                      className="px-3 py-1.5 text-xs font-medium text-neutral-400 hover:text-white hover:bg-neutral-800 rounded-lg transition-colors border border-transparent hover:border-neutral-700"
                      title="Reset view"
                    >
                      Reset
                    </button>
                  ...
                  onWheel={(e) => {
                    ...
                    if (pdfTransformRef.current) {
                      const { positionX, positionY, scale } =
                        pdfTransformRef.current.instance.transformState;
                      ...
                      pdfTransformRef.current.setTransform(
                        positionX,
                        newY,
                        scale,
                      );
            ...
            <div
              className={`flex-none relative bg-neutral-900 border-t border-neutral-800 transition-all duration-300 ease-in-out ${showThumbnails ? "h-24" : "h-0"}`}
            >
              {/* Toggle Tab Button */}
              <button
              ...
              <div
                className={`flex gap-2 h-full items-center justify-center w-full overflow-x-auto scrollbar-thin scrollbar-thumb-neutral-700 scrollbar-track-neutral-900 px-4 py-2 ${showThumbnails ? "opacity-100" : "opacity-0 pointer-events-none"}`}
              >
                {sortedEvidences.map((ev, idx) => {
                  const fileUrl = `/api/proxy-file?path=${ev.file_path}`;
                  const isPdf = ev.file_path.toLowerCase().endsWith(".pdf");
                  const isSelected =
                    selectedImage === fileUrl || selectedPdf === fileUrl;
                  ...
                  return (
                    <button
                      ...
                      className={`relative group h-14 w-14 flex-shrink-0 rounded-md overflow-hidden border transition-all duration-200 ${
                        isSelected
                    ...
                    >
                      {isPdf ? (
                        <div className="w-full h-full bg-neutral-800 flex flex-col items-center justify-center p-0.5">
                          ...
                          <span className="text-[8px] text-neutral-400 font-mono mt-0.5">
                            PDF
                          </span>
                        </div>
                      ) : (
      ...
      {(selectedImage || selectedPdf) && currentItem && (
        <StickyInfoBox
          ...
          receivedDate={new Date(currentItem.received_date).toLocaleDateString(
            "id-ID",
            {
              weekday: "long",
              year: "numeric",
