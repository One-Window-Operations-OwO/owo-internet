    ...
    <div className="space-y-8">
      ...
      <div className="grid gap-6 md:grid-cols-2 lg:grid-cols-5">
        <div className="rounded-2xl bg-white dark:bg-zinc-900 p-6 shadow-sm border border-neutral-200 dark:border-zinc-800 transition-colors">
          ...
          <p className="mt-2 text-3xl font-bold text-neutral-900 dark:text-white">{data.total_users}</p>
        </div>
        <div className="rounded-2xl bg-white dark:bg-zinc-900 p-6 shadow-sm border border-neutral-200 dark:border-zinc-800 transition-colors">
      ...
      <div className="mt-12 mb-4 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        ...
        <div className="flex flex-col sm:flex-row gap-3">
          ...
          <button
            onClick={() => {
              const headers = ["Nama User", "Total Data", "Pemeriksaan Fisik Sesuai", "Sudah Dikerjakan", "Belum Dikerjakan"];
              const rows = data.user_kpis
                .filter(kpi => kpi.name && kpi.name.toLowerCase().includes(searchTerm.toLowerCase()))
                .map(kpi => [
                  kpi.name,
                  kpi.total_data,
              ...
              link.setAttribute("download", `user_kpis_${new Date().toISOString().slice(0, 10)}.csv`);
          ...
          <table className="min-w-full divide-y divide-neutral-200 dark:divide-zinc-800">
            ...
            <tbody className="divide-y divide-neutral-100 dark:divide-zinc-800/50">
              {data.user_kpis
                .filter(kpi => kpi.name && kpi.name.toLowerCase().includes(searchTerm.toLowerCase()))
                .map((kpi) => {
                  const sudahDikerjakan = kpi.verifikasi_selesai + kpi.verifikasi_ditolak;
                  return (
                    <tr
                      key={kpi.id}
                    ...
                    >
                      ...
                      <td className="px-6 py-4 text-right bg-emerald-50/30 dark:bg-emerald-900/5">
                        <div className="flex flex-col items-end">
                          ...
                          <span className="text-xs text-neutral-400">
                            {kpi.total_data > 0
                              ? (((kpi.verifikasi_selesai || 0) / kpi.total_data) * 100).toFixed(1)
                              : 0}
                            %
                          </span>
                        </div>
                      ...
                      <td className="px-6 py-4 text-right">
                        <div className="flex flex-col items-end">
                          ...
                          <span className="text-xs text-neutral-400">
                            {kpi.total_data > 0
                              ? (((sudahDikerjakan || 0) / kpi.total_data) * 100).toFixed(1)
                              : 0}
                            %
                          </span>
                        </div>
              ...
              {data.user_kpis.filter(kpi => kpi.name && kpi.name.toLowerCase().includes(searchTerm.toLowerCase())).length === 0 && (
                <tr>
