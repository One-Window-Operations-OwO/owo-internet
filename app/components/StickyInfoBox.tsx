...
export default function StickyInfoBox({
  ...
  return (
    ...
    >
      ...
      >
        <span className="font-bold text-yellow-500 text-sm break-words whitespace-normal w-full">
          {schoolName || "-"}
        </span>
      </div>
      ...
      >
        ...
        <div>
          ...
          <div className="text-xs text-white">{resiNumber || "-"}</div>
        </div>
        ...
        <div>
          ...
          <div className="text-xs text-white">{receivedDate || "-"}</div>
        </div>
            ...
            (history && history.length > 0 ? (
              <div className="space-y-3">
                {history.map((log: any, idx: number) => {
                  ...
                  const rejections =
                    log.rejections && typeof log.rejections === "object"
                      ? Object.entries(log.rejections)
                          .map(([k, v]) => `${k}: ${v}`)
                          .join(", ")
                      : null;
